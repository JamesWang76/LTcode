...
/**
 * Find a largest set such that all the elements in this set could divide each other
 * 
 * - DP concept
 *      - 2 for-loop, one (i) is for all elements and the other (j) is for all remain elements bigger than the current index i.
 *      - If j could divided by i, then i is the previous node of j and set the j's ctr by set the i's ctr+1
 * 
 * 93% / 100%
 */ 
...
vector<int> largestDivisibleSubset(vector<int> nums) {
    ...
    int traceIdx = maxNodeIdx;
    for (int i = 0; i < maxSize; i++) {
        ans.insert(ans.begin(), nums[traceIdx]);
        traceIdx = arr[traceIdx].lastNodeIdx;
    }
    return ans;
}